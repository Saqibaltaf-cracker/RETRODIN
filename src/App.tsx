    ...
    <div className={`w-screen min-w-[100vw] max-w-[100vw] ${isolated ? 'fixed inset-0 h-[100dvh] min-h-[100dvh] max-h-[100dvh] z-40 overflow-hidden bg-black' : 'min-h-screen overflow-x-hidden bg-[#0a0a0c]'} flex flex-col items-center ${isolated ? 'justify-center p-0 m-0' : 'justify-between'} relative font-sans select-none`}>
      ...
      {isolated ? (
        ...
        <div className="fixed top-2.5 sm:top-3 right-2.5 sm:right-3 z-50 flex items-center gap-1.5 sm:gap-2">
          ...
          <button
            ...
            className={`px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg text-[10.5px] sm:text-xs font-mono font-bold tracking-wider uppercase border transition-all duration-300 backdrop-blur-md cursor-pointer flex items-center gap-1.5 shadow-lg active:scale-95 ${
              stereo.showSetupMenu 
                ? 'bg-[var(--color-lcd-primary)]/20 border-[var(--color-lcd-primary)] text-white shadow-[0_0_12px_rgba(var(--color-lcd-primary-rgb),0.5)]'
          ...
          >
            <Settings className={`w-3.5 h-3.5 ${stereo.showSetupMenu ? 'text-[var(--color-lcd-primary)]' : perfSettings.lowEndMode ? 'text-amber-400 animate-spin-slow' : 'text-zinc-400'}`} />

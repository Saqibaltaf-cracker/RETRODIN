...
export const SpectrumAnalyzer: React.FC<Props> = ({ 
  ...
  displayMode = 'oled',
  matrixResolution = '64x32',
  oledResolution = '1080P',
  screenBrightness = 1.0,
  visualizerBrightness
}) => {
  const activeBrightness = visualizerBrightness ?? screenBrightness ?? 1.0;
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const bootStartTimeRef = useRef<number>(0);
  const prevBootingRef = useRef<boolean>(false);

  // Active resolution refs to ensure instantaneous frame updates
  const matrixResRef = useRef(matrixResolution);
  matrixResRef.current = matrixResolution;
  const oledResRef = useRef(oledResolution);
  oledResRef.current = oledResolution;
  ...
  const oledOffscreenRef = useRef<HTMLCanvasElement | null>(null);
  ...
  useEffect(() => {
    ...
    const oledOffscreen = document.createElement('canvas');
    oledOffscreenRef.current = oledOffscreen;
  ...
  useEffect(() => {
    ...
    const needsDrift = visualizerMode === 'JDM_CAR_DOTS';
    const needsTandem = visualizerMode === 'JDM_TANDEM_DOTS' || visualizerMode === 'JDM_CAR_OLED';
  ...
  useEffect(() => {
    ...
    const renderDotMatrixFromVideo = (
      ...
      // Dynamic matrix resolution with exact grid steps (32x16, 48x24, 64x32, 80x40, 128x64)
      let cols = 64;
      let rows = 32;
      const res = matrixResRef.current;
      if (res === '32x16' || res === 'low') {
        cols = 32;
        rows = 16;
      } else if (res === '48x24' || res === 'med') {
        cols = 48;
        rows = 24;
      } else if (res === '80x40' || res === 'ultra') {
        cols = 80;
        rows = 40;
      } else if (res === '128x64') {
        cols = 128;
        ...
        cols = isLowEndVideo ? 32 : 64;
        rows = isLowEndVideo ? 16 : 32;
      }
      ...
      const dotScale = cols === 32 ? 1.7 : cols === 48 ? 1.3 : cols === 64 ? 1.0 : cols === 80 ? 0.8 : 0.52;
      const maxDotR = cols === 32 ? 3.4 : cols === 48 ? 2.4 : cols === 64 ? 1.8 : cols === 80 ? 1.4 : 0.92;
      const minDotR = cols === 32 ? 0.9 : cols === 48 ? 0.7 : cols === 64 ? 0.5 : cols === 80 ? 0.4 : 0.28;
      const offDot = cols >= 128 ? 0.6 : (cols >= 80 ? 0.8 : 1.0);
          ...
          try {
            ...
            for (let r = 0; r < rows; r++) {
              ...
              for (let c = 0; c < cols; c++) {
                ...
                if (intensity > 0.14) {
                  const dotRadius = Math.max(minDotR, Math.min(maxDotR, intensity * 1.7 * dotScale));
                  const dotAlpha = Math.min(1, intensity * 1.25) * effectiveDimmer;
                  ctx.fillStyle = hexToRgba(colors.primary, dotAlpha);
                  if (intensity > 0.68 && allowGlow && cols <= 80) {
                  ...
                  ctx.fillRect(px - offDot / 2, py - offDot / 2, offDot, offDot);
                }
              }
            }
            rendered = true;
        ...
        for (let r = 0; r < rows; r++) {
          ...
          for (let c = 0; c < cols; c++) {
            ...
            if (dist <= 2) {
              ...
              ctx.beginPath();
              ctx.arc(px, py, Math.max(0.6, 1.2 * dotScale), 0, Math.PI * 2);
              ctx.fill();
            } else {
              ctx.fillStyle = hexToRgba(colors.primaryOff, 0.12 * effectiveDimmer);
              ctx.fillRect(px - offDot / 2, py - offDot / 2, offDot, offDot);
    ...
    const draw = () => {
      ...
      const effectiveDimmer = dimmerLevel * Math.max(0.15, activeBrightness);
      const isRgb = false;
      ...
      // MODE 3: DOT MATRIX (Small circular phosphor dots scaled to matrix resolution)
      if (visualizerMode === 'DOT_MATRIX') {
        const mRes = matrixResRef.current;
        let bandsCount = 64;
        let segsCount = 32;
        let dR = 1.2;

        if (mRes === '32x16' || mRes === 'low') {
          bandsCount = 32;
          segsCount = 16;
          dR = 2.4;
        } else if (mRes === '48x24' || mRes === 'med') {
          bandsCount = 48;
          segsCount = 24;
          dR = 1.7;
        } else if (mRes === '64x32' || mRes === 'high') {
          bandsCount = 64;
          segsCount = 32;
          dR = 1.2;
        } else if (mRes === '80x40' || mRes === 'ultra') {
          bandsCount = 80;
          segsCount = 40;
          dR = 0.95;
        } else if (mRes === '128x64') {
        ...
        for (let i = 0; i < bandsCount; i++) {
          ...
          const dataIdx = Math.floor((i / bandsCount) * Math.min(128, dataArray.length));
          const val = dataArray[dataIdx] || 0;
          const litSegments = powered ? Math.min(segsCount, Math.floor((val / 255) * (segsCount - 0.5))) : 0;

          for (let j = 0; j < segsCount; j++) {
            const cy = height - (j * ((height - 6) / segsCount)) - 4;
            ...
            if (isRgb) {
              ...
              const topThreshold = Math.floor(segsCount * 0.8);
              const midThreshold = Math.floor(segsCount * 0.6);
              dotColor = j >= topThreshold ? colors.danger : (j >= midThreshold ? colors.secondary : colors.primary);
              dotColorOff = j >= topThreshold ? colors.dangerOff : (j >= midThreshold ? colors.secondaryOff : colors.primaryOff);
            }

            // Clean natural dot matrix illumination
            ctx.fillStyle = isLit ? (isRgb ? dotColor : hexToRgba(dotColor, Math.min(1.0, effectiveDimmer * 1.1))) : (isRgb ? dotColorOff : hexToRgba(dotColorOff, 0.35));
            ctx.shadowColor = isLit ? dotColor : 'transparent';
            ctx.shadowBlur = isLit && allowGlow && bandsCount <= 64 ? 3 : 0;
            ctx.beginPath();
            ctx.arc(cx, cy, dR, 0, Math.PI * 2);
            ctx.fill();

            if (isLit && bandsCount <= 48) {
              ctx.shadowBlur = 0;
      ...
      if (visualizerMode === 'FIRE_SPECTRUM') {
        let fireBands = numBands;
        if (displayModeRef.current === 'matrix') {
          const mRes = matrixResRef.current;
          if (mRes === '32x16' || mRes === 'low') fireBands = 32;
          else if (mRes === '48x24' || mRes === 'med') fireBands = 48;
          else if (mRes === '64x32' || mRes === 'high') fireBands = 64;
          else if (mRes === '80x40' || mRes === 'ultra') fireBands = 80;
          else if (mRes === '128x64') fireBands = 128;
        }

        const bSpacing = fireBands >= 80 ? 0.8 : (fireBands >= 48 ? 1.2 : 1.5);
        const fBandWidth = (width / fireBands) - bSpacing;

        for (let i = 0; i < fireBands; i++) {
          const x = i * (fBandWidth + bSpacing) + 1;
          const dataIdx = Math.floor((i / fireBands) * Math.min(64, dataArray.length));
          const val = dataArray[dataIdx] || 0;
          ...
          if (litHeight > 0) {
            ...
            if (isRgb) {
              const bHue = ((i / fireBands) * 310 + frameCounter) % 360;
            ...
            ctx.fillStyle = grad;
            ctx.shadowBlur = allowGlow && fireBands <= 64 ? 4 : 0;
            ctx.fillRect(x, height - litHeight, fBandWidth, litHeight);

            // Flickering flame tip spark
            if (Math.random() > 0.4) {
              ctx.fillStyle = '#ffffff';
              ctx.fillRect(x, height - litHeight - (Math.random() * 4 + 1), fBandWidth, 1.5);
            }
          }
        }
        ctx.shadowBlur = 0;
      ...
      // Helper to render OLED video with selectable resolution scaling (480P, 720P, 1080P)
      const renderOledVideo = (videoElem: HTMLVideoElement | null) => {
        // Pure OLED Black base
        ctx.fillStyle = '#000000';
        ctx.fillRect(0, 0, width, height);

        if (videoElem && videoElem.readyState >= 2) {
          const oRes = oledResRef.current;
          const oledOff = oledOffscreenRef.current;

          if ((oRes === '480P' || oRes === 'low') && oledOff) {
            // 480P retro pixel resolution (120x60) with un-smoothed pixel scaling
            if (oledOff.width !== 120 || oledOff.height !== 60) {
              oledOff.width = 120;
              oledOff.height = 60;
            }
            const oledCtx = oledOff.getContext('2d');
            if (oledCtx) {
              oledCtx.drawImage(videoElem, 0, 0, 120, 60);
              ctx.imageSmoothingEnabled = false;
              ctx.drawImage(oledOff, 0, 0, 120, 60, 0, 0, width, height);
              ctx.imageSmoothingEnabled = true;
            } else {
              ctx.drawImage(videoElem, 0, 0, width, height);
            }
          } else if ((oRes === '720P' || oRes === 'med') && oledOff) {
            // 720P balanced medium resolution (180x90) with smooth scaling
            if (oledOff.width !== 180 || oledOff.height !== 90) {
              oledOff.width = 180;
              oledOff.height = 90;
            }
            const oledCtx = oledOff.getContext('2d');
            if (oledCtx) {
              oledCtx.drawImage(videoElem, 0, 0, 180, 90);
              ctx.imageSmoothingEnabled = true;
              ctx.drawImage(oledOff, 0, 0, 180, 90, 0, 0, width, height);
            } else {
              ctx.drawImage(videoElem, 0, 0, width, height);
            }
          } else {
            // 1080P native razor-sharp full-resolution direct render
            ctx.imageSmoothingEnabled = true;
            ctx.drawImage(videoElem, 0, 0, width, height);
          }
        } else {
      ...
      if (visualizerMode === 'SERENE_JAPAN') {
        ...
        if (displayModeRef.current === 'matrix') {
          const offscreen = offscreenCanvasRef.current;
          renderDotMatrixFromVideo(sv, ctx, width, height, offscreen, effectiveDimmer, now);
        } else {
          renderOledVideo(sv);
        }
        return;
      ...
      // Dynamic band and segment count based on matrix resolution in Matrix mode
      let activeNumBands = numBands;
      let activeSegments = segmentsPerBand;

      if (displayModeRef.current === 'matrix') {
        const mRes = matrixResRef.current;
        if (mRes === '32x16' || mRes === 'low') { activeNumBands = 32; activeSegments = 16; }
        else if (mRes === '48x24' || mRes === 'med') { activeNumBands = 48; activeSegments = 24; }
        else if (mRes === '64x32' || mRes === 'high') { activeNumBands = 64; activeSegments = 32; }
        else if (mRes === '80x40' || mRes === 'ultra') { activeNumBands = 80; activeSegments = 40; }
        else if (mRes === '128x64') { activeNumBands = 128; activeSegments = 64; }
      }

      const activeBandSpacing = activeNumBands >= 80 ? 0.8 : (activeNumBands >= 48 ? 1.0 : 1.5);
      const activeBandWidth = (width / activeNumBands) - activeBandSpacing;
      const activeSegSpacing = activeSegments >= 40 ? 0.8 : 1.0;
      const activeSegmentHeight = (height / activeSegments) - activeSegSpacing;

      for (let i = 0; i < activeNumBands; i++) {
        const x = i * (activeBandWidth + activeBandSpacing) + 1;
        const dataIdx = Math.floor((i / activeNumBands) * Math.min(64, dataArray.length));
        const val = dataArray[dataIdx] || 0;
        
        // Dynamic intensity: authentic vintage response without ceiling pinning
        const targetSegments = (powered && !isBooting) 
          ? Math.min(activeSegments, Math.floor((val / 255) * (activeSegments - 0.5))) 
          : 0;
        
        // Peak hold physics
        const peakIdx = i % numBands;
        if (!isBooting) {
          if (targetSegments >= peakHold[peakIdx]) {
            peakHold[peakIdx] = targetSegments;
            peakHoldTime[peakIdx] = visualizerMode === 'PEAK_FALL' ? 24 : 16;
          } else {
            if (peakHoldTime[peakIdx] > 0) {
              peakHoldTime[peakIdx]--;
            } else {
              peakHold[peakIdx] = Math.max(0, peakHold[peakIdx] - (visualizerMode === 'PEAK_FALL' ? 0.2 : 0.35));
            }
          }
        }

        const litSegments = targetSegments;
        const topThreshold = Math.floor(activeSegments * 0.8);
        const midThreshold = Math.floor(activeSegments * 0.6);

        for (let j = 0; j < activeSegments; j++) {
          const y = height - (j * (activeSegmentHeight + activeSegSpacing)) - activeSegmentHeight - 1;
          
          let isLit = j < litSegments;
          const isPeak = Math.floor(peakHold[peakIdx]) === j && powered && peakHold[peakIdx] > 0;
          ...
          if (visualizerMode === 'PEAK_FALL') {
            ...
            if (isPeak) {
              ctx.fillStyle = '#ffffff';
              ctx.shadowBlur = 6;
              ctx.shadowColor = isRgb ? colorOn : (j >= topThreshold ? colors.danger : colorOn);
              ctx.fillRect(x, y, activeBandWidth, activeSegmentHeight);
            } else if (isLit) {
              ctx.fillStyle = isRgb ? colorOn : hexToRgba(colorOn, 0.88);
              ctx.shadowBlur = allowGlow && activeNumBands <= 64 ? (j >= midThreshold ? 3.5 : 2) : 0;
              ctx.shadowColor = colorOn;
              ctx.fillRect(x, y, activeBandWidth, activeSegmentHeight);
              if (j >= midThreshold && allowGlow && activeNumBands <= 64) {
                ctx.fillStyle = hexToRgba('#ffffff', (j >= topThreshold ? 0.4 : 0.25) * effectiveDimmer);
                ctx.fillRect(x, y, activeBandWidth, 1);
              }
            } else {
            ...
            ctx.shadowBlur = (isLit || isPeak && allowGlow && activeNumBands <= 64) ? (isPeak ? 6 : (isRgb ? 4 : (j >= midThreshold ? 5 : 3))) : 0;
            ctx.shadowColor = (isLit || isPeak && allowGlow && activeNumBands <= 64) ? (isPeak ? '#ffffff' : (j >= topThreshold ? colors.danger : colorOn)) : 'transparent';
            ctx.fillRect(x, y, activeBandWidth, activeSegmentHeight);

            // Fluorescent crisp highlight on upper lit segments so they stand out brightly
            if (isLit && j >= midThreshold && allowGlow && activeNumBands <= 64) {
              ctx.fillStyle = hexToRgba('#ffffff', (j >= topThreshold ? 0.45 : 0.28) * effectiveDimmer);
              ctx.fillRect(x, y, activeBandWidth, 1);
            }
          }
        }
  ...
  }, [engine, powered, dimmerLevel, isBooting, isYtPlaying, playing, theme, visualizerMode, fpsLimit, canvasGlow, lowEndMode, simplifiedDisplayOnIdle, displayMode, matrixResolution, oledResolution, screenBrightness]);

  // Real-time canvas internal raster dimensions based on OLED / Matrix resolution settings
  let canvasWidth = 270;
  ...
  if (isMobileClient) {
    ...
    if (oledResolution === '480P') {
      ...
      canvasHeight = 90;
      isPixelated = true;
    } else if (oledResolution === '720P') {
      canvasWidth = 270;
      canvasHeight = 134;
      isPixelated = false;
    } else {
      // 1080P: High-resolution Full HD
      canvasWidth = 540;
      canvasHeight = 268;
      isPixelated = false;
    }
  ...
  // Shadow mask aperture grid size matching matrix resolution
  const maskGridSize = matrixResolution === '32x16' 
    ? '8px 8px' 
    : matrixResolution === '48x24' 
    ? '5.5px 5.5px' 
    : matrixResolution === '64x32' 
    ? '4px 4px' 
    : matrixResolution === '80x40' 
    ? '3.2px 3.2px' 
    : '2px 2px';
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <canvas 
        ...
        width={canvasWidth} 
        height={canvasHeight} 
        className="w-full h-full block transition-[filter] duration-150"
        style={{
          imageRendering: isPixelated ? 'pixelated' : 'auto',
          filter: activeBrightness !== 1.0 ? `brightness(${activeBrightness})` : undefined
        }}
      />

...
export interface PerformanceSettings {
  ...
  matrixResolution: '32x16' | '48x24' | '64x32' | '80x40' | '128x64'; // Matrix grid density
  oledResolution: '480P' | '720P' | '1080P'; // OLED video resolution scale
  screenBrightness: number; // 0.25 to 1.0 (25% to 100%)
  visualizerBrightness: number; // 0.25 to 1.25 (25% to 125%)
...
export function getDefaultSettings(isLowEnd: boolean): PerformanceSettings {
  if (isLowEnd) {
    return {
      ...
      vintageSilver: false,
      displayMode: 'oled',
      matrixResolution: '32x16',
      oledResolution: '480P',
      screenBrightness: 1.0,
      visualizerBrightness: 1.0,
...
export function usePerformanceSettings() {
  ...
  const [settings, setSettings] = useState<PerformanceSettings>(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (parsed.matrixResolution === 'low') parsed.matrixResolution = '32x16';
        if (parsed.matrixResolution === 'med') parsed.matrixResolution = '48x24';
        if (parsed.matrixResolution === 'high') parsed.matrixResolution = '64x32';
        if (parsed.matrixResolution === 'ultra') parsed.matrixResolution = '80x40';
        if (!['32x16', '48x24', '64x32', '80x40', '128x64'].includes(parsed.matrixResolution)) {
          parsed.matrixResolution = '64x32';
        }

        if (parsed.oledResolution === 'low') parsed.oledResolution = '480P';
        if (parsed.oledResolution === 'med') parsed.oledResolution = '720P';
        if (parsed.oledResolution === 'high') parsed.oledResolution = '1080P';
        if (!['480P', '720P', '1080P'].includes(parsed.oledResolution)) {
          parsed.oledResolution = '1080P';
        }
  ...
  const updateSetting = useCallback(<K extends keyof PerformanceSettings>(key: K, value: PerformanceSettings[K]) => {
    setSettings(prev => {
      ...
      if (key === 'visualizerBrightness') {
        updated.screenBrightness = value as number;
      } else if (key === 'screenBrightness') {
        updated.visualizerBrightness = value as number;
      }
      return updated;
    });
  }, []);

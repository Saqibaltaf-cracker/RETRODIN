...
const ALL_THEMES: { id: StereoTheme; label: string }[] = [
  ...
  { id: 'full-white', label: 'FULL MONO WHITE' },
...
export function useRecoverySettings({
  ...
  const recoveryItems: RecoverySettingItem[] = useMemo(() => [
    ...
    },
    {
      ...
      value: (() => {
        const r = perfSettings.matrixResolution ?? '64x32';
        if (r === 'low' as any) return '32x16';
        if (r === 'med' as any) return '48x24';
        if (r === 'high' as any) return '64x32';
        if (r === 'ultra' as any) return '80x40';
        return r;
      })(),
      onNext: () => {
        const steps = ['32x16', '48x24', '64x32', '80x40', '128x64'] as const;
        let cur = perfSettings.matrixResolution ?? '64x32';
        if (cur === 'low' as any) cur = '32x16';
        if (cur === 'med' as any) cur = '48x24';
        if (cur === 'high' as any) cur = '64x32';
        if (cur === 'ultra' as any) cur = '80x40';
        const curIdx = steps.indexOf(cur as any);
        const next = steps[(curIdx + 1) % steps.length];
        updateSetting('matrixResolution', next);
        stereo.showToast(`MATRIX: ${next}`);
      },
      onPrev: () => {
        const steps = ['32x16', '48x24', '64x32', '80x40', '128x64'] as const;
        let cur = perfSettings.matrixResolution ?? '64x32';
        ...
        updateSetting('matrixResolution', prev);
        stereo.showToast(`MATRIX: ${prev}`);
      }
    },
    {
      id: 'oledResolution',
      label: 'OLED RESOLUTION',
      value: (() => {
        const r = perfSettings.oledResolution ?? '1080P';
        if (r === 'low' as any) return '480P';
        if (r === 'med' as any) return '720P';
        if (r === 'high' as any) return '1080P';
        return r;
      })(),
      onNext: () => {
        const steps = ['480P', '720P', '1080P'] as const;
        let cur = perfSettings.oledResolution ?? '1080P';
        if (cur === 'low' as any) cur = '480P';
        if (cur === 'med' as any) cur = '720P';
        if (cur === 'high' as any) cur = '1080P';
        const curIdx = steps.indexOf(cur as any);
        const next = steps[(curIdx + 1) % steps.length];
        updateSetting('oledResolution', next);
        stereo.showToast(`OLED: ${next}`);
      },
      onPrev: () => {
        const steps = ['480P', '720P', '1080P'] as const;
        let cur = perfSettings.oledResolution ?? '1080P';
        ...
        const curIdx = steps.indexOf(cur as any);
        const prev = steps[(curIdx - 1 + steps.length) % steps.length];
        updateSetting('oledResolution', prev);
        stereo.showToast(`OLED: ${prev}`);
      }
    },
    {
      id: 'brightness',
      ...
      value: (() => {
        const val = perfSettings.visualizerBrightness ?? perfSettings.screenBrightness ?? 1.0;
        return `${Math.round(val * 100)}%`;
      })(),
      onNext: () => {
        const steps = [0.25, 0.5, 0.75, 1.0, 1.25];
        const cur = perfSettings.visualizerBrightness ?? perfSettings.screenBrightness ?? 1.0;
        const curIdx = steps.findIndex(s => Math.abs(s - cur) < 0.05);
        const next = steps[(curIdx + 1) % steps.length];
        updateSetting('visualizerBrightness', next);
        stereo.showToast(`VIS BRIGHTNESS: ${Math.round(next * 100)}%`);
      },
      onPrev: () => {
        const steps = [0.25, 0.5, 0.75, 1.0, 1.25];
        const cur = perfSettings.visualizerBrightness ?? perfSettings.screenBrightness ?? 1.0;
        const curIdx = steps.findIndex(s => Math.abs(s - cur) < 0.05);
        const prev = steps[(curIdx - 1 + steps.length) % steps.length];
        updateSetting('visualizerBrightness', prev);
        stereo.showToast(`VIS BRIGHTNESS: ${Math.round(prev * 100)}%`);
      }
    },
    {
      id: 'fpsLimit',
